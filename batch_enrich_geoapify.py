#!/usr/bin/env python3
"""
Batch Geoapify Places Enrichment Script

This script processes all raw store JSON files through the Geoapify Places enrichment pipeline.
It automatically skips brands that have already been Geoapify enriched.

Usage: python batch_enrich_geoapify.py
"""
...
ENRICH_SCRIPT = Path("scripts/enrich_with_geoapify.py")
...
def is_already_enriched(brand_name):
    """Check if brand has already been Geoapify enriched"""
    # Check for various possible output filename patterns
    possible_names = [
        f"{brand_name}_geoapify.json",
        f"{brand_name}_geoapify_enriched.json",
        f"{brand_name}_google.json",  # Also check for google enriched (same dir)
        f"{brand_name}.json"
    ]

    for name in possible_names:
        if (PLACES_ENRICHED_DIR / name).exists():
            return True
    return False

def main():
    """Process all raw JSON files through Geoapify Places enrichment"""
    ...
    PLACES_ENRICHED_DIR.mkdir(parents=True, exist_ok=True)
    ...
    print("=" * 80)
    print("BATCH GEOAPIFY PLACES ENRICHMENT")
    print("=" * 80)
    ...
    print("=" * 80)
    print()
    print("⚠️  NOTE: Geoapify Places API has rate limits and costs.")
    ...
    for i, (brand_name, raw_file) in enumerate(brands_to_process, 1):
        ...
        output_file = PLACES_ENRICHED_DIR / f"{brand_name}_geoapify.json"
    ...
    if success_count > 0:
        print(f"📁 Enriched files saved to: {PLACES_ENRICHED_DIR}")
